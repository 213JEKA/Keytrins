...
public class MainActivity extends android.app.Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BotRuntime.log("Приложение готово. Реальная торговля по умолчанию выключена.");
        refreshUiLoop();
    }
    ...
    private void confirmLive() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage("Робот сможет отправлять реальные ордера Bybit. Базовый риск — 3 USDT на валидный вход. API-ключ не должен иметь право вывода средств.\n\nЧтобы включить, введите LIVE.")
                .setView(input)
    ...
    private void doctor() {
        ...
        statusText.setText("Проверка API…");
        worker.submit(() -> {
            try {
                SettingsStore.Snapshot s = store.load();
    ...
    private void startBot() {
        ...
        if (s.live && !BotRuntime.liveArmed) {
            Toast.makeText(this, "LIVE не подтверждён. Выключите и снова включите переключатель LIVE, затем введите LIVE.", Toast.LENGTH_LONG).show();
            return;
        }
        if (s.live && (s.apiKey.isEmpty() || s.apiSecret.isEmpty())) {
            Toast.makeText(this, "Для LIVE нужны API key + secret", Toast.LENGTH_LONG).show();
            return;
        }
