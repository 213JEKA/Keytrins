...
final class RiskState {
    ...
    void rollDay(double equity){String d=today(); if(!d.equals(p.getString(pre+"day","")) || p.getFloat(pre+"dayEquity",0f)<=0f){p.edit().putString(pre+"day",d).putFloat(pre+"dayEquity",(float)equity).putString(pre+"haltDay","").apply();}}
