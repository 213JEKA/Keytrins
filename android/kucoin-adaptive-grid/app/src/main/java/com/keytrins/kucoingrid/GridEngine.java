...
public final class GridEngine implements Runnable {
 ...
 private void cycle() throws Exception {
  ...
  List<ApiClient.Candle> m1=api.klines(cfg.symbol,60,120); if(m1.size()<60)throw new Exception("Недостаточно свечей"); ApiClient.ContractInfo ci=api.contract(cfg.symbol);
  ...
  log.add(String.format(Locale.US,"%s  %.6f | ATR %.2f%% ADX %.1f | шаг %.2f%%",mode,price,atrPct*100,adx,stepPct*100));
