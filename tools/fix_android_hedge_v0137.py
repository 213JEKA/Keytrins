...
# Replace hedge decision block in managePositions with exchange-truth-first logic.
...
# Hedge trigger uses fixed entry ATR plus a later fail-safe backstop at -0.15R.
replace_once(
    'app/src/main/java/com/keytrins/liveresearch/bot/LiveResearchEngine.java',
...
# Do not misclassify a freshly opened hedge as closed because the current positions snapshot predates the order.
replace_once(
    'app/src/main/java/com/keytrins/liveresearch/bot/LiveResearchEngine.java',
...
# Add retry/recovery helpers before hedgeStopImproves.
...
# Add software catch-up exits when an intended dollar floor has already been crossed before exchange stop placement.
...
# Catch-up helper methods before maybeReduce.
...
# Dashboard shows entry ATR trigger so failures are visible immediately.
replace_once(
...
# Settings text and version.
replace_once(
    'app/src/main/res/layout/activity_settings.xml',
    'Dollar lock: каждые +0.50 USDT по максимуму → защита +0.50 USDT с отставанием 0.50',
    'Hedge: противоход от −0.15 entry ATR • закрыть hedge при возврате primary в чистый плюс\\nDollar lock: каждые +0.50 USDT по максимуму → защита +0.50 USDT с отставанием 0.50'
)
